package dev.ivanqueiroz.kanbanfx.view.components;

import dev.ivanqueiroz.kanbanfx.view.components.skins.MultiColumnListViewSkin;
 ...
 * {@link ListView}. The control allows the user to rearrange the items in each {@link ListView} and
 * also to drag and drop items from one column to another.
 *
...
public class MultiColumnListView<T> extends Control {
  ...
  public String getUserAgentStylesheet() {
    return Objects.requireNonNull(
            MultiColumnListView.class.getResource("multi-column-list-view.css"))
        .toExternalForm();
  }
  ...
  /**
   * Determines whether the headers will be shown or not. Toggling this property will trigger a
   * rebuild of the view.
   *
  ...
  private final ObjectProperty<Callback<MultiColumnListView<T>, ListView<T>>> listViewFactory =
      new SimpleObjectProperty<>(this, "listViewFactory", m -> new AutoscrollListView<>());
   ...
   */
  public final ObjectProperty<Callback<MultiColumnListView<T>, ListView<T>>>
      listViewFactoryProperty() {
    return listViewFactory;
  }

  public final void setListViewFactory(
      Callback<MultiColumnListView<T>, ListView<T>> listViewFactory) {
    this.listViewFactory.set(listViewFactory);
  }

  private final ListProperty<ListViewColumn<T>> columns =
      new SimpleListProperty<>(this, "columns", FXCollections.observableArrayList());
  ...
  /**
   * A list of columns that define how many columns will be shown inside the view. The model objects
   * in this list also store the header and the data for each column.
   *
  ...
  private final ObjectProperty<Callback<MultiColumnListView<T>, ColumnListCell<T>>> cellFactory =
      new SimpleObjectProperty<>(this, "cellFactory", ColumnListCell::new);
   ...
   */
  public final ObjectProperty<Callback<MultiColumnListView<T>, ColumnListCell<T>>>
      cellFactoryProperty() {
    return cellFactory;
  }

  public final void setCellFactory(
      Callback<MultiColumnListView<T>, ColumnListCell<T>> cellFactory) {
    this.cellFactory.set(cellFactory);
  }

  private final ObjectProperty<Callback<Integer, Node>> separatorFactory =
  ...
  /**
   * An optional factory for creating separators that will be placed between columns. The default
   * implementation creates a region and adds the style class "column-separator". No separators will
   * be added to the view when the factory is being set to null.
   *
  ...
  private final BooleanProperty disableDragAndDrop =
      new SimpleBooleanProperty(this, "disableDragAndDrop");
  ...
  public static class ListViewColumn<T> {
    ...
    private final ObjectProperty<Node> header =
        new SimpleObjectProperty<>(this, "header", new Label("Column Header"));
  ...
  private final ObjectProperty<T> placeholderFrom =
      new SimpleObjectProperty<>(this, "placeholderFrom");
  ...
  /**
   * A special list cell to be used in combination with the {@link MultiColumnListView} control. The
   * cell adds drag and drop support for re-arranging list cells and for dragging them from one
   * column to another.
   *
  ...
  public static class ColumnListCell<T> extends ListCell<T> {
     ...
     *
     * @param multiColumnListView reference to the {@link MultiColumnListView} control where the
     *     cell is being used
     */
    public ColumnListCell(MultiColumnListView<T> multiColumnListView) {
      ...
      setOnDragDetected(
          event -> {
            ...
            multiColumnListView
                .getDraggedItems()
                .setAll(getListView().getSelectionModel().getSelectedItems());
      ...
      setOnDragOver(
          event -> {
            ...
            if (event.getGestureSource() != this
                && multiColumnListView.getPlaceholderFrom() != getItem()) {
              log.debug("   accepting, " + hashCode() + ", txt: " + getText());
              updateItems(event);
              event.consume();
              event.acceptTransferModes(TransferMode.MOVE);
            } else {
              log.debug("   not accepting drag");
              event.acceptTransferModes(TransferMode.NONE);
            }
          });

      setOnDragEntered(event -> log.debug("drag entered"));
      ...
      setOnDragDropped(
          event -> {
            ...
            if (multiColumnListView.getPlaceholderFrom() == getItem()) {
              log.debug("   not performing drop, drop happened on 'from' placeholder");
              return;
            }

            log.debug("   performing drop");
            ...
            T draggedDroppedItem = multiColumnListView.getDraggedItem();
            items.replaceAll(
                item -> {
                  if (item == multiColumnListView.getPlaceholderTo()) {
                    log.debug("   replace all from drop");
                    return draggedDroppedItem;
                  }
                  return item;
                });

            listView.getSelectionModel().select(draggedDroppedItem);
      ...
      setOnDragDone(
          evt -> {
            ...
            if (Objects.equals(evt.getAcceptedTransferMode(), TransferMode.MOVE)) {
              log.debug(
                  "   drop was completed, removing the 'from' placeholder. getPlaceholderFrom {}",
                  multiColumnListView.getPlaceholderFrom());
              getListView()
                  .getItems()
                  .removeIf(
                      item -> {
                        var result = (item == multiColumnListView.getPlaceholderFrom());
                        log.debug("removeIf {}", result);
                        return result;
                      });
            } else {
              log.debug("   drop was not completed, replacing placeholder with dragged item");
              getListView()
                  .getItems()
                  .replaceAll(
                      item -> {
                        if (item == multiColumnListView.getPlaceholderFrom()) {
                          log.debug("   item equals to getPlaceholderFrom");
                          return multiColumnListView.getDraggedItem();
                        }
                        log.debug("   item not equals to getPlaceholderFrom");
                        return item;
                      });
            }
    ...
    /**
     * Returns the {@link MultiColumnListView} control where the cell is being used.
     *
    ...
    private void updateItems(DragEvent event) {
      if (event.getGestureSource() != this) {
        ...
        log.debug("updateItems - toIndex {}", toIndex);
        ...
        items.remove(toItem);
        log.debug("item count now: " + items.size());

        if (event.getY() < getHeight() / 2) {
          log.debug("   attempt to add ABOVE");
          if (toIndex > 0) {
            int finalToIndex = Math.min(toIndex, items.size());
            if (notNextToEachOther(fromIndex, finalToIndex)) {
              log.debug("      adding 'to' placeholder at index " + toIndex);
              items.add(finalToIndex, toItem);
            }
          } else {
            if (notNextToEachOther(fromIndex, 0)) {
              log.debug("      adding 'to' placeholder at index 0");
              items.add(0, toItem);
            }
          }
        } else {
          log.debug("   attempt to add BELOW");
          if (toIndex < items.size() - 1) {
            int finalToIndex = toIndex + 1;
            if (notNextToEachOther(fromIndex, finalToIndex)) {
              log.debug("      adding 'to' placeholder at index " + finalToIndex);
              items.add(finalToIndex, toItem);
            }
          } else {
    ...
    private boolean notNextToEachOther(int fromIndex, int toIndex) {
      ...
      // special checks.
      log.debug("from {} / to index: {} ",fromIndex, toIndex);
    ...
    private void updateDraggedPseudoState() {
      ...
      pseudoClassStateChanged(
          PseudoClass.getPseudoClass("from"), from != null && from == getItem());
    ...
    private final ReadOnlyBooleanWrapper placeholder =
        new ReadOnlyBooleanWrapper(this, "placeholder");
    ...
    /**
     * A read-only property that is being set to true if the item in the cell is currently either
     * one of the two placeholder items (see {@link MultiColumnListView#placeholderFromProperty()}
    ...
    private final ReadOnlyBooleanWrapper fromPlaceholder =
        new ReadOnlyBooleanWrapper(this, "fromPlaceholder");
    ...
    /**
     * A read-only property that is being set to true if the item in the cell is currently the
     * "from" placeholder item (see {@link MultiColumnListView#placeholderFromProperty()}).
     *
    ...
    private final ReadOnlyBooleanWrapper toPlaceholder =
        new ReadOnlyBooleanWrapper(this, "toPlaceholder");
    ...
    /**
     * A read-only property that is being set to true if the item in the cell is currently the "to"
     * placeholder item (see {@link MultiColumnListView#placeholderToProperty()}).
     *
