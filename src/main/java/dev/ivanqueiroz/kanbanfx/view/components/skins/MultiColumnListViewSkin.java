package dev.ivanqueiroz.kanbanfx.view.components.skins;
...
public class MultiColumnListViewSkin<T> extends SkinBase<MultiColumnListView<T>> {
  ...
  private void updateView() {
    ...
    do {
      ...
      listView
          .placeholderProperty()
          .addListener(
              (obs, oldPlaceholder, newPlaceholder) -> {
  ...
  private void initPlaceholder(ListView listView, Node placeholder) {
    placeholder.setOnDragOver(
        event -> {
          event.consume();
          event.acceptTransferModes(TransferMode.MOVE);
        });

    placeholder.setOnDragDropped(
        event -> {
    ...
    placeholder.setOnDragEntered(
        evt -> placeholder.pseudoClassStateChanged(PseudoClass.getPseudoClass("drag-over"), true));
    placeholder.setOnDragExited(
        evt -> placeholder.pseudoClassStateChanged(PseudoClass.getPseudoClass("drag-over"), false));
  }
}
